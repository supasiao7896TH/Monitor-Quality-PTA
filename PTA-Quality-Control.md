...
✅ **กริด Severity / Correlation Matrix (ส่วนที่ 3) ดึงข้อมูลครบและยืนยันความถูกต้องแล้ว** — สัญลักษณ์ ◎○▷ ในไฟล์ต้นฉบับเป็นรูปวาด/shape ไม่ใช่ text ในเซลล์ (`xlrd` อ่านไม่ได้) จึงใช้ Excel COM automation ดึงพิกัดจาก `Worksheet.Shapes` collection โดยตรงแทน ตรวจสอบไขว้กับข้อมูลที่พี่ A ยืนยันจาก SOP เองแล้ว 16/16 คู่ (ดูรายละเอียดวิธีอ่าน + ประวัติการแก้บั๊กที่ [ส่วนที่ 3](#3-correlation-matrix-factor--item-severity) และ [Source](#source))
...
หมายเหตุ: กริดนี้มี 12 Item (ไม่มี **THM** เทียบกับตารางที่ 1 ซึ่งมี 13 Item) — พี่ A ยืนยันแล้ว (2026-08-10) ว่าตั้งใจไม่รวม THM ไว้ในการวิเคราะห์ correlation นี้ ไม่ใช่ตกหล่น จึงไม่มี factor ใดจับคู่กับ THM ได้ในฟีเจอร์ auto-suggest — ถือเป็นข้อจำกัดที่รู้อยู่แล้ว ไม่ใช่บั๊ก
